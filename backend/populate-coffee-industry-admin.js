...
async function populateCoffeeIndustryAdmin() {
  console.log('🌱 Populating database with Coffee Industry data (Admin Mode)...\n');

  try {
    // First, let's check if we can connect and see existing data
    console.log('🔍 Checking database connection...');
    const { data: existingChains, error: chainError } = await supabase
      .from('coffee_chains')
      .select('*')
      .limit(5);
    
    if (chainError) {
      console.log('❌ Error checking coffee_chains table:', chainError.message);
    } else {
      console.log(`✅ Found ${existingChains?.length || 0} existing coffee chains`);
    }
    ...
    const { data: finalChains, error: finalChainError } = await supabase
      .from('coffee_chains')
      .select('chain_name, total_outlets, primary_location')
      .limit(10);
    
    if (finalChainError) {
      console.log('❌ Error verifying coffee chains:', finalChainError.message);
    } else {
      console.log('✅ Coffee chains in database:');
      finalChains?.forEach(chain => {
